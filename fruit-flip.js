document.addEventListener('DOMContentLoaded', () => {
  ...
  function replaceMatchedCards(card1, card2) {
    ...
    setTimeout(() => {
      ...
      // Find all cards that are not currently matched
      const unpairedCards = cards.filter(c => c !== card1 && c !== card2);
      
      // If there are other cards on the board, implement the swap mechanism
      if (unpairedCards.length > 0) {
        ...
        // Create a clone of the randomly chosen card
        const clonedSwapCard = createCardElement(cardToSwap.dataset.fruit, `clone_${Date.now()}`);
        
        console.log('Swapping cards with new implementation');
        
        // Place first new card where first matched card was
        gameBoard.replaceChild(newCard1, card1);
        
        // Place cloned card where second matched card was
        gameBoard.replaceChild(clonedSwapCard, card2);
        
        // Place second new card where the random card was
        gameBoard.replaceChild(newCard2, cardToSwap);
        
        // Update cards array
        cards = cards.map(card => {
          if (card === card1) return newCard1;
          if (card === card2) return clonedSwapCard;
          if (card === cardToSwap) return newCard2;
          return card;
        });
      } else {
        // Fallback to original behavior if no other cards to swap
        gameBoard.replaceChild(newCard1, card1);
